    ...
    <div className="min-h-screen bg-[#F8F5F0] text-[#1A1A1A] font-sans selection:bg-[#C5A059] selection:text-white">
      ...
      <header className="relative w-full h-[85vh] flex items-center justify-center overflow-hidden text-white">
        ...
        <div className="relative z-10 text-center px-6">
          ...
          <motion.h1 initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="text-6xl md:text-9xl font-serif font-extrabold mb-8 leading-tight tracking-tighter">Forging Your <br /> Own Legacy.</motion.h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {tourData.map((tour) => (
            <motion.div key={tour.id} whileHover={{ y: -10 }} className="group relative h-[700px] rounded-[48px] overflow-hidden shadow-2xl bg-stone-900">
              <Image src={tour.image} alt={tour.title} fill className="object-cover opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-1000" />
      ...
      <section className="py-24 px-6 bg-white border-b border-black/5 text-[#1A1A1A]">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl md:text-5xl font-serif font-bold mb-6 leading-tight text-[#1A1A1A]">Bringing <span className="text-[#C5A059]">Clarity</span> to the Legend.</h2>
      ...
      <section className="py-32 px-6 bg-[#F8F5F0]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-20">
            ...
            <h2 className="text-5xl font-serif font-bold tracking-tight text-[#1A1A1A]">Interactive Heartland Map</h2>
          </div>
          <InteractiveMap />
        </div>
      </section>
